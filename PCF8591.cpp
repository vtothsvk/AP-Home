...
uint8_t PCF8591::adc_raw_read(uint8_t channel){
	...
	Wire.write(0x40 | (channel & 0b11));//select channel
	...
	Wire.requestFrom(_i2c_addr, 2);//request data
	Wire.read();//dump status byte
	retval = Wire.read();//read data

	if(_power_save and not _dac_enable){
		dac_shutdown();//power off oscilator
	}
...
{
	Wire.beginTransmission(_i2c_addr);
	Wire.write(0x44);//sellect all channel sequetial read
	...
	Wire.requestFrom(_i2c_addr, 5);//request data
	Wire.read();//dump status byte
	res[0] = Wire.read();//read adc[0] value
	res[1] = Wire.read();//...
	res[2] = Wire.read();//...
	res[3] = Wire.read();//...
